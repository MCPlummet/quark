...
- **No secrets in this repo.** The APNs `.p8`, its key id, and the ansible vault live outside the tree. This repo is public.

---

## What is already done — do not rebuild

**Phase 0 (infrastructure) — complete and verified 2026-08-19.** Sygnal 0.17.0 runs on the VPS behind `push.quark.tel`, deployed from `~/ansible/hgorg` (`roles/system/tasks/quark-sygnal.yml`, `templates/sygnal.yaml.j2`). It defines both `tel.quark.app.ios.dev` and `tel.quark.app.ios.prod` against one APNs auth key, with `send_badge_counts: false` because the homeserver cannot count encrypted rooms. Verified live:
...
- [ ] Confirm the APNs auth key in the ansible vault is still valid and its Key ID matches `apns_key_id` in `~/ansible/hgorg/group_vars/all/vars.yml`.
...
```bash
ssh <vps> 'docker logs --tail 100 sygnal 2>&1 | grep -i "reject\|error"'
```
