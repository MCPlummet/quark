...
Quark's version string lives in **seven** places. The project guidance (CLAUDE.md, README) only names three — the README shields.io badge, the `Cargo.lock` `quark` entry, the iOS `Info.plist`, and the xcodegen `project.yml` are easy to forget, and they have historically been bumped late in separate commits (the plist drifted to `0.11.3` while the tree was at `0.12.0`; `project.yml` silently sat at `0.5.0` until the tree reached `0.17.2`, because nothing updated *or* checked it). This skill changes all seven together and refuses to run if they're already out of sync.
...
| 7 | `src-tauri/gen/apple/project.yml` | `CFBundleShortVersionString` **and** `CFBundleVersion` — both carry the plain semver, but with different quoting (`: X.Y.Z` bare vs `: "X.Y.Z"`), which the script preserves. xcodegen stamps these into the built iOS app, so they must match the plist. Checked independently: either key can drift alone |

## When to bump (timing)

**Bump once per branch, when prepping it for merge — not eagerly while the feature is still in progress.** A single branch often lands several feature/fix commits; bumping on each one churns the seven version files, produces noisy intermediate versions, and is the main source of the cross-file drift this skill exists to prevent. So:
...
2. **Aborts if the seven files don't already agree** — printing which one drifted (naming the specific `project.yml` key, since its two are checked separately), so you reconcile an existing mismatch rather than burying it.
...
5. Rewrites all seven with scoped substitutions (dependency versions in Cargo.toml/Cargo.lock, and the iOS deployment target in project.yml, are left alone).
